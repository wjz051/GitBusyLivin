...
bool GBL_Repository::clone_repo(GBL_String srcUrl, GBL_String dstPath)
{
    ...
    try
    {
        ...
        clone_opts.fetch_opts.callbacks.credentials = git_cred_acquire_cb(cred_acquire_cb);
        //clone_opts.fetch_opts.callbacks.payload = &pd;

        check_libgit_return(git_clone(&m_pRepo, srcUrl.toConstChar(), dstPath.toConstChar(), &clone_opts));
...
bool GBL_Repository::push_to_remote(GBL_String sRemote, GBL_String sBranch)
{
    ...
    try
    {
        ...
        check_libgit_return(git_remote_lookup(&pRemote, m_pRepo, sRemote.toConstChar()));
         ...
         //git_push_init_options( &options, GIT_REMOTE_CALLBACKS_INIT );
         options.callbacks.credentials = git_cred_acquire_cb(cred_acquire_cb);
...
int GBL_Repository::cred_acquire_cb(git_cred **out, const char *url, const char *username_from_url, unsigned int allowed_types, void *payload)
{
    return git_cred_userpass_plaintext_new(out, GBL_String("k0790004").toConstChar(), GBL_String("Ht@123456").toConstChar());
    //return git_cred_ssh_key_new(out,username_from_url,nullptr,GBL_String("D:/Users/K0090013/.ssh/id_rsa").toConstChar(),"21152211");
}
